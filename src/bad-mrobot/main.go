...
func LoadG729() []byte {
	...
	if err != nil {
		fmt.Printf("ReadFile ", ring, " error ", err)
		return nil
	}
...
func arguments() {
	local, _ = net.ResolveIPAddr("ip", os.Args[1])
	localPort, _ = strconv.Atoi(os.Args[2])

	remote, _ = net.ResolveIPAddr("ip", os.Args[3])
	remotePort, _ = strconv.Atoi(os.Args[4])
}
